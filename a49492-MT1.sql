...
SELECT E.IDE AS IDT, E.Data_Evento AS Data_Tour, E.Nome_Evento AS Designacao, P.IDL, 
       (SELECT Nome_Local FROM Lugar WHERE IDL = P.IDL) AS Nome_Local,
       (SELECT Info FROM Lugar WHERE IDL = P.IDL) AS Info
FROM Evento E, Percurso P WHERE E.IDE = P.IDE 
  AND P.IDL IN (SELECT IDL FROM Lugar WHERE Nome_Local LIKE 'Barr%'  AND Info IS NOT NULL );
